...
//This function draws a polygon shape which will be used
//as the shape of the stars.

//The reason for this function is that a custom polygon
//will look more star like than a basic shape like a circle.

//The x and y inputs are the coordinates of the shape centre
//radius is distance from centre to each 'star' point
...
function drawPolygon(x, y, radius, starPoints) {
    ...
    //Start creating the custom shape

    //REFERENCE NOTE: The method for making a custom shape was learned from p5 references:
    //https://p5js.org/reference/p5/beginShape/

    //Connects vertices defined between here and endShape
    beginShape();
    ...
    for (let a = 0; a < TWO_PI; a += angle) {
        ...
        //This holds the x coordinate of the star point
        let starX = x + cos(a) * radius;

        //REFERENCE NOTE: The use of sin() was studied from p5 references:
        //https://p5js.org/reference/p5/sin/

        //This holds the y coordinate of the star point
        let starY = y + sin(a) * radius;

        //Create a new vertex for the star shape using the
        //above coordinates
        vertex(starX, starY);
    }

    //Finish the custom shape (connects last point to first point)
    endShape(CLOSE);
}
...
//It only needs to be done once so is called in sketch.js setup()
function setupStars() {

    //Iterate numStars times to create that number of stars and put them in stars array
    for(let i = 0; i < numStars; i++) {

        //Add object to stars array:
        stars.push({
...
//The stars will fade as the sky stress value lowers.
function drawStars() {

     //This value controls how transparent the stars are
     //As sky stress increases they become more transparent
    let alpha = map(skyStress, 0, 0.5, 255, 0, true);
    ...
    for (let star of stars) {
        ...
        //This is the horizontal position value of where the star will be drawn
        let starX = fit.x + col * (fit.w / numSegments);
        //This is the vertical position value of where the star will be drawn
        ...
        //Draw the star as a custom polygon using the drawPolygon function
        //Use the above starX and starY values for x/y position of the star
        //Multiply size by 0.4 to keep stars a reasonable size
        //6 for a 6 point star
        drawPolygon(starX, starY, size * 0.4, 6);
    }
...
//It is used to update the flowing wiggle movement of the sky
